...
oauthrouter = APIRouter(prefix="/oauth", tags=["Authentication"])
...
@oauthrouter.post("/login", response_model=TokenResponse, responses=AUTH_RESPONSES)
...
@router.post("/refresh", response_model=AccessTokenResponse, responses=AUTH_RESPONSES)
@limiter.limit("5/minute")
def refresh_token(
...
@router.post(
    "/logout", status_code=status.HTTP_204_NO_CONTENT, responses=PROTECTED_RESPONSES
)
@limiter.limit("5/minute")
def logout(
    request: RefreshTokenRequest,
